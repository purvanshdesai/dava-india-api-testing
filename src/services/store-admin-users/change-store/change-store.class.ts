...
import { handleStoreChanges } from '../../super-admin-users/change-store/change-store.shared'
...
export interface StoreAdminUsersChangeStoreServiceOptions {
  app: Application
}

export interface StoreAdminUsersChangeStoreParams extends Params<StoreAdminUsersChangeStoreQuery> {}

// This is a skeleton for a custom service class. Remove or add the methods you need here
export class StoreAdminUsersChangeStoreService {
  constructor(public options: StoreAdminUsersChangeStoreServiceOptions) {}
  ...
  async find(params?: any): Promise<any> {
    ...
    const orderTracking: any = await OrderItemTrackingModal.findOne({
      ...
      .populate('items')
      .lean()
    // const productIds = orderTracking.items.map((i: any) => i.product)

    const deliveryPolicy = await DeliveryPoliciesModel.findOne({ stores: store }).lean()
    const stores: any = await StoreModel.find({
      _id: {
        $in: deliveryPolicy?.stores?.filter((item) => item?.toString() != store?.toString())
      },
      active: true
    }).lean()
  ...
  async update(id: NullableId, data: StoreAdminUsersChangeStoreData, _params?: any): Promise<any> {
    return {
      id: 0,
      ...data
    }
  ...
  async patch(id: NullableId, data: StoreAdminUsersChangeStorePatch, _params?: any): Promise<any> {
    return {
      id: 0,
...
export class StoreAdminPartialOrderTransferService {
  constructor(public options: StoreAdminUsersChangeStoreServiceOptions) {}
